...
BYTE Structure::getMixColEntry(int row, int column)
{
	return mixColMatrix[row][column];
}

BYTE Structure::getSboxEntry(int row, int column)
{
	return BYTE(sbox[row][column]);
}


BYTE Structure::getSboxEntry(BYTE bin)
{
	int row =	BYTE(bin.to_string().substr(0, 4)).to_ulong();
	int col =	BYTE(bin.to_string().substr(4, 8)).to_ulong();
...
void Structure::printRow(BYTE *row)
{
	for (int col = 0; col < 4; col++)
		printf("%s ", row[col].to_string().c_str());
	printf("\n");
}
...
{
	vector<BYTE> column;
	column.push_back(matrix[0][colNum]);
	column.push_back(matrix[1][colNum]);
	column.push_back(matrix[2][colNum]);
